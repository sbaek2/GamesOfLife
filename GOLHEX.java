...
public class GOLHEX 
{
    ...
    {
        //A dot is used to signal for alive life.
        String dot = "•";
        int n = alive.length*2;
        int [][]grid = new int[n][n];
        int rowCount=0;
        double i=0.0;
        double j=-0.5;
        
        //Printing the results with the hexagonal grid
        while (rowCount<grid.length)
        {
            if (rowCount%4==0)
            {
    ...
    {
        //Establishing a 2d array neighs with 6*2 (Hexagonal)
        int [][]neighs= new int[6][2];
        int count=0;
        boolean subtractChecker=false;
        ...
        for (int y=j-1; y<=j+1; y++)
        {
            for (int x=i; x<=i+1; x++)
            {
                if (y==j&&subtractChecker==false)
                {
                    x=i-1;
                    subtractChecker=true;
                }
